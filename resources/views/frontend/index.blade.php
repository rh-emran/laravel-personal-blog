            ...
            @foreach($articles as $article)
                <div class="flex justify-between max-w-screen-xl px-4 py-8 mx-auto lg:gap-12 xl:gap-12 lg:py-12 border-b">
                    <div class="w-full lg:w-1/2 {{ $loop->even ? 'order-2' : ''  }}">
                        ...
                        <p class="max-w-2xl mb-2 font-light text-gray-400 lg:mb-2 md:text-sm lg:text-sm dark:text-gray-500">Category:
                            <span class="bg-blue-100 text-blue-800 text-md font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{{ $article->category->name }}</span>
                        </p>
                        <p class="max-w-2xl mb-6 font-light text-gray-400 lg:mb-8 md:text-sm lg:text-sm dark:text-gray-500">Tags:
                            @foreach($article->tags as $tag)
                                <span class="bg-green-100 text-green-800 text-sm leading-8 font-medium px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-200">{{ $tag->name }}</span>
                            @endforeach
                        </p>
                        <a href="{{ route('article.details', $article->id) }}" class="inline-flex items-center justify-center px-5 py-3 mr-3 text-base font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:focus:ring-primary-900">
                            Read More
